...
public final class MimeTypeUtils {
	...
	public static final String MIMETYPE_DOCUMENT = "document";
	public static final String MIMETYPE_COMPRESSION = "compression";
	...
	public static String getCategoryMimeType(String path) {
		String extension = path.substring(path.lastIndexOf(".") + 1,
				path.length()).toLowerCase();
		if (extension != null) {
			String type = sMimeTypeMap.get(extension);
			if (type != null) {
				if (type.startsWith(MIMETYPE_AUDIO)
						&& !type.equals("audio/x-pn-realaudio")) {
					return MIMETYPE_AUDIO;
				} else if (type.startsWith(MIMETYPE_VIDEO)
						|| type.equals("audio/x-pn-realaudio")) {
					return MIMETYPE_VIDEO;
				} else if (type.startsWith(MIMETYPE_IMAGE)) {
					...
					return MIMETYPE_APK;
						...
						|| type.equals("application/rar")
						|| type.equals("application/x-tar")
						|| type.equals("application/x-iso9660-image")
						|| type.equals("appication/x-compress")
						|| type.equals("application/x-gzip")
						|| type.equals("application/x-gtar")) {
					return MIMETYPE_COMPRESSION;
				}				
